<template>
  ...
  <div class="bookRead" :style="{ background: `rgba(${backColor[isBcolor]}`+(range/100)+')' }">
    ...
    <div class="Popup" v-if="isShow" @click="isShow=false">
      ...
      <div class="Catalog" @click.stop="" :style="{height:clientH+'px'}" v-if="isSCont==2">
        <div class="catalogTit">
          ...
          <div class="name">
            <h1>{{detail.name}}</h1>
            <h2>{{detail.author}} <span>{{chapterList.length>0?(pageNum+1/chapterList.length*100).toFixed(1):0}}%已读</span></h2>
          </div>
          <img src="../../assets/img/detail/close.png" alt=""  @click="isShow=false">
        </div>

        <div :class="{list:true,lock:detail.price&&itm.pid.indexOf('txt001')<0?true:false}"  v-for="(itm,inx) in chapterList" :key="inx" @click="changDirectory(itm,inx)" v-if="chapterList.length>0">
    ...
    <div class="first" v-if="isFirst">
      <ul>
        <li><img src="../../assets/img/detail/icon_middle.png" alt=""><h1>向上滑动显示下一页</h1></li>
        <!--<li class="middle"><img src="../../assets/img/detail/icon_middle.png" alt=""><h1>点击中间或向上滑动展示功能菜单</h1></li>-->
        <li><img src="../../assets/img/detail/icon_bottom.png" alt=""><h1>向下滑动显示上一页</h1></li>
      </ul>
      <div class="button" @click="firstFn">
        我知道了
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        isBcolor:0,
        isFirst:Cookies.get('isFirst')=='false'?false:true,
        detail:{price:0,},
    ...
    created() {
      ...
      this.clientH=document.body.clientHeight-(document.body.clientWidth*80/375);
      this.winH=document.documentElement.clientHeight+(winW*180/375);
      this.getDetail();
    },
    watch: {},
    computed:{
    ...
    methods: {
      ...
      touchend(ev){
        ...
        /*console.log('结束',ev.changedTouches);*/
        if((this.endY-this.startY>100)&&this.pageNum>0&&scrollTop==0){
