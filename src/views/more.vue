      ...
      <ul v-if="(curModular==0||curModular==2)&&list.length>0" class="therapyList">
        <li v-for="(item,index) in list" :key="index">
          ...
          <div :class="item.img?'info css1':'info'">
            ...
            <p :id="`description${index}`">{{Ellipsis(`description${index}`, 2, item.synopsis||item.description, '.19')}}</p>
            ...
            getListAPI(data).then(res => {
              if (res.data.status) {
                ...
                /*res.data.data.pageNum = val.pageNum;*/
                  ...
                  console.log('禁用',self.list.length<self.total)
                  if(self.list.length<self.total){
                    self.disabled=false;
                  }else{
                    self.disabled=true;
                  }
                  console.log(22222,self.list)
